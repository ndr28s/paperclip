    ...
    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("workspaceControls.run")
          : action === "start"
            ? t("workspaceControls.start")
            : action === "stop"
              ? t("workspaceControls.stop")
              : t("workspaceControls.restart");
...
export function WorkspaceRuntimeControls({
  ...
  const resolvedServiceEmptyMessage = emptyMessage ?? serviceEmptyMessage ?? t("workspaceControls.noServices");
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className="rounded-xl border border-border/70 bg-background/60 p-3">
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceControls.workspaceCommands")}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0 ? t("workspaceControls.servicesRunning", { count: runningCount }) : t("workspaceControls.noServicesRunning")}
            </span>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? t("workspaceControls.jobsAvailable", { count: resolvedSections.jobs.length })
                : t("workspaceControls.commandsControlled")}
            </span>
          </div>
      ...
      <CommandSection
        title={t("workspaceControls.services")}
        description={t("workspaceControls.servicesDesc")}
      ...
      <CommandSection
        title={t("workspaceControls.jobs")}
        description={t("workspaceControls.jobsDesc")}
        items={resolvedSections.jobs}
        emptyMessage={jobEmptyMessage ?? t("workspaceControls.noJobs")}
        isPending={isPending}
        pendingRequest={pendingRequest}
        onAction={onAction}
      />
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("workspaceControls.untrackedServices")}
          description={t("workspaceControls.untrackedServicesDesc")}
